package org.ncu.healthcare.aspect;
...
public class LoggingAspect {
    ...
    @Before("execution(* org.ncu.healthcare.*.*.*(..)) && !execution(* org.ncu.healthcare.aop.*.*(..))")
    public void logBeforeAllMethodsExceptAOP(JoinPoint joinPoint) {
        System.out.println("Before calling method: " + joinPoint.getSignature().toShortString());
    }

    @Before("within(org.ncu.healthcare.aspect..*)")
    ...
    @After("execution(* org.ncu.healthcare.*.*.*(..))")
    public void logAfterAllMethods(JoinPoint joinPoint) {
        System.out.println("After calling method: " + joinPoint.getSignature().toShortString());
    }

    @AfterThrowing(pointcut = "execution(* org.ncu.healthcare.*.*.*(..))", throwing = "ex")
    ...
    @AfterReturning(pointcut = "execution(* org.ncu.healthcare.*.*.*(..))", returning = "result")
    ...
    @Around("execution(* org.ncu.healthcare.*.*.*(..))")
